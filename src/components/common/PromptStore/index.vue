...
const mobileDataSource = computed(() => {
...
watch(
  ...
  (newVal) => {
    if (unref(newVal))
      promptStore.getCategoryDetail(promptStore.selectIndex)
  },
  { deep: true },
)

const changeTab = async (value: number) => {
  promptStore.getCategoryDetail(value)
}
</script>
...
<template>
  <NModal v-model:show="show" style="width: 90%; max-width: 1400px" title="角色商店" preset="card">
    <div class="space-y-4">
      ...
      >
        <NTabPane
          v-for="element in promptStore.CategoryList"
          :key="element.scene_id"
          :name="element.scene_id"
          :tab="element.scene_name"
        >
          <div v-if="!isMobile" class="typeContainer">
            <NCard
            ...
            >
              {{ item.value }}
              <template #action>
                <NButton @click="changeShowModal('modify', item)">
                  编辑
                </NButton>
              </template>
            </NCard>
          </div>
          <NList v-else style="max-height: 400px; overflow-y: auto">
            <NListItem v-for="(item, index) of mobileDataSource" :key="index">
